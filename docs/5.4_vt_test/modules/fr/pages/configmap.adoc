...
{product-name} prend en charge la configuration automatisée à l'aide de la fonction ConfigMap de Kubernetes. Cela permet de déployer de manière automatisée les conteneurs {product-name} avec les configurations, intégrations et autres paramètres appropriés.

Le paramètre 'always_reload : true' peut être ajouté dans n'importe quel yaml de ConfigMap pour forcer le rechargement de ce yaml à chaque fois que le contrôleur démarre (version 4.3.2+). Sinon, le ConfigMap ne sera chargé qu'au démarrage initial ou après un redémarrage complet du cluster (voir la section sur le stockage persistant ci-dessous).

=== Exemple complet {product-name} ConfigMap (initcfg.yaml)

La dernière version du ConfigMap est disponible à https://raw.githubusercontent.com/neuvector/manifests/main/kubernetes/5.4.0/initcfg.yaml[l'adresse suivante :].

L'échantillon est également présenté ci-dessous. Elle contient tous les paramètres disponibles. Veuillez supprimer les sections inutiles et modifier les sections nécessaires. 

[NOTE]
====
Si vous utilisez configmap dans un secret, voir la section ci-dessous pour les changements de format.
====
...
Créez ensuite l'objet ConfigMap :
...
=== Exemples de cartes de configuration fédérées (fedinitcfg.yaml)

NeuVector v5.4.0 supporte l'automatisation de la Fédération à travers le ConfigMap. Vous trouverez ci-dessous des exemples de configurations `fedinitcfg.yaml` qui peuvent être appliquées à vos clusters primaires et gérés en fonction de votre cas d'utilisation.

Exemple `fedinitcfg.yaml` pour le cluster primaire :
...
Exemple `fedinitcfg.yaml` pour les clusters gérés :
...
== Protéger les données sensibles à l'aide d'un secret

Si des données sensibles doivent être incluses dans certaines sections de la carte de configuration, un secret peut être créé pour les sections contenant des données sensibles.

Par exemple, créez le configMap pour les sections NON sensibles telles que passwordProfile et role :
...
Créez ensuite un secret pour les sections contenant des données sensibles, par exemple :
...
[IMPORTANT]
====
Supprimez le caractère "|" dans chaque section, comme indiqué ci-dessous.
====

Notez la suppression du caractère "pipe" ci-dessous si vous utilisez des sections de la carte de configuration dans un secret, si l'activation est réglée sur "true" et si vous décommentez la section à inclure dans le secret.
...
Une fois le contrôleur déployé, tous les fichiers de configuration de configmap et de secret seront stockés dans le dossier /etc/config.

Notez que le secret est mentionné dans les xref:kubernetes.adoc#_deploy_suse_security[fichiers yaml de déploiement] standard xref:kubernetes.adoc#_deploy_suse_security[de] Kubernetes et OpenShift Controller sous Volumes.

== ConfigMaps et stockage permanent

Les ConfigMaps et la sauvegarde du xref:production.adoc#_backups_and_persistent_data[stockage permanent] ne sont lus que lorsqu'un nouveau cluster {product-name} est déployé, ou lorsque le cluster tombe en panne et est redémarré. Ils ne sont pas utilisés lors des mises à niveau en continu.
