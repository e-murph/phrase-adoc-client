...
image:3taskdef.png[Tâche Allinone]
...
--
. Créez un nouveau service pour la tâche Enforcer. Une fois encore, définissez le placement des tâches sur "`One Task Per Host`" afin qu'un seul Enforcer soit déployé sur chaque hôte. Notez également que la contrainte supplémentaire doit montrer qu'elle empêche le déploiement sur un nœud allinone.
+
--
