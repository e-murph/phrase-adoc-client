...
Afin d'exécuter l'analyse de vulnérabilité en mode autonome, le plugin Jenkins doit tirer l'image du scanner vers l'hôte où l'agent s'exécute, vous devez donc entrer l'URL du registre du scanner {product-name}, le référentiel d'image, et l'identifiant si nécessaire, dans la page de configuration du plugin {product-name}.
...
image:jenkins5a.png[canalisation]
