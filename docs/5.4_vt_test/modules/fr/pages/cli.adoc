...
== Utilisation de la ligne de commande {product-name} 

La solution {product-name} prend en charge un ensemble limité de fonctions à l'aide de la CLI. La CLI est prise en charge par le gestionnaire, qui utilise à son tour une RestAPI pour envoyer des commandes au contrôleur. Le contrôleur gère ensuite le(s) agent(s) d'exécution de manière appropriée. Un ensemble complet d'opérations est pris en charge par l'API REST, qui peut être exposée directement à partir du contrôleur. Vous pouvez accéder à la CLI de {product-name} en tapant la commande cli pour le Manager ou Allinone, par exemple :
...
Où '`allinone`' est le nom du conteneur du contrôleur. Il peut être nécessaire d'utiliser l'ID du conteneur pour le nom.

Bien que le CLI soit disponible via le gestionnaire, nous recommandons d'utiliser l'xref:automation.adoc[API REST] directement dans le contrôleur pour l'interrogation et l'automatisation.

=== Exemples de commandes CLI

Voici quelques-unes des commandes CLI les plus courantes :
...
Utilisez le même utilisateur/mot de passe que pour la console.
...
D'autres commandes CLI sont énumérées ci-dessous.

== Référence et commandes de la ligne de commande
...
[NOTE]
====
Les fichiers de reniflage sont stockés dans le répertoire /var/neuvector/pcap du conteneur Enforcer. Veillez à mapper le volume dans le répertoire de votre machine invitée ou dans le répertoire du système local pour pouvoir accéder aux fichiers. Par exemple, dans le fichier docker-compose, ajoutez '`- /var/neuvector:/var/neuvector`' dans les volumes.
====

Pour démarrer la capture de paquets sur un pod, vous aurez besoin de connaître l'ID du container à passer dans le champ ID_OR_NAME. Vous pouvez le faire avec `show container -c <container_name>`. puis démarrer le renifleur avec `request sniffer start <container_id>`. Par exemple,
...
[IMPORTANT]
====
Si la durée n'est pas définie, vous devrez trouver l'ID du renifleur afin d'arrêter le renifleur.  Pour ce faire, `show sniffer -c <containerID>`.  Suivre par `request sniffer stop <sniffer_ID>`.
====
