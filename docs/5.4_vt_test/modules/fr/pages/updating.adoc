...
Si le déploiement a été effectué à l'aide des tableaux {product-name} Helm, la mise à jour s'occupera des services supplémentaires, des liaisons de rôles ou d'autres exigences de mise à niveau.
...
Pour les utilisateurs de Helm, mettez à jour vers {product-name} Helm chart 2.0.0 ou plus récent (avant {product-name} 5.2.0). En cas de mise à jour d'une installation Operator ou Helm sur OpenShift, voir la note ci-dessous.
...
Les images se trouvent sur le registre {product-name} Docker Hub. Utilisez la balise de version appropriée pour le gestionnaire, le contrôleur, l'exécuteur, et laissez la version "dernière" pour l'analyseur et l'actualisateur. Par exemple :
