...
Créez le service Redis et le déploiement à l'aide de ce fichier yaml :
...
Ensuite, connectez-vous à l'adresse IP publique/au port de l'un des nœuds kubernetes, par ex. `http://(public_IP):(NodePort)`
