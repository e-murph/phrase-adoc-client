...
Für die tokenbasierte Authentifizierung siehe die folgenden Screenshots und den Beispielaufruf. Achten Sie darauf, das Geheimnis und den Token nach der Erstellung zu kopieren, da es keine Möglichkeit gibt, diese nach dem Schließen des Bildschirms abzurufen.
...
Das nachstehende Beispielskript zeigt, wie man den Container aus der Ferne abruft, ihn ausführt und scannt. Es kann von einer Jenkins-Aufgabe (Remote Shell) oder einem beliebigen CI/CD-Tool ausgelöst werden. Ein JSON-Parser-Tool (jq) wird ebenfalls verwendet.
