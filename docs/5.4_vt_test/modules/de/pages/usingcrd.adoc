...
* Gruppennamen wie nv.node-pod.demo werden im CRD zwar referenziert, aber nicht definiert, so dass davon ausgegangen wird, dass sie bei der Bereitstellung bereits existieren. Siehe unten zur Definition von Gruppen.
...
Richtlinienmodus Konfiguration Yaml-Datei Beispiel
...
[NOTE]
====
Beachten Sie, dass für diese Clusterrolle 2 Ressourcen konfiguriert wurden: nvsecurityrules und nvclustersecurityrules. Example (nvsecnvclustroles.yaml):
