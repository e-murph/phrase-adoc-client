...
Die Images befinden sich in der {product-name} Docker Hub Registry. Verwenden Sie das entsprechende Versions-Tag für Manager, Controller und Enforcer und belassen Sie die Version für Scanner und Updater bei "latest". Zum Beispiel:
...
[NOTE]
====
Wenn beim Erstellen eines Projekts das Argument --node-selector verwendet wird, schränkt dies die Pod-Platzierung, z. B. für den {product-name} enforcer, auf bestimmte Knoten ein.
====
...
Wenn es zusätzliche Färbungen wie oben gibt, fügen Sie diese in den Abschnitt "Muster-Yaml-Toleranzen" ein:
...
Fügen Sie dann einen nodeSelector in die yaml-Datei für die Manager- und Controller-Bereitstellungsabschnitte ein. Zum Beispiel:
