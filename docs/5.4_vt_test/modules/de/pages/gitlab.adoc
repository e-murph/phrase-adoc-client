...
== Scannen nach Schwachstellen während der Gitlab Build Pipeline

{product-name} kann so konfiguriert werden, dass es nach Schwachstellen sucht, die in der Gitlab-Build-Pipeline ausgelöst werden.  gibt es ein https://gitlab.com/neuvector/gitlab-plugin[Hier], das konfiguriert und verwendet werden kann. Bitte befolgen Sie die Anweisungen auf der Gitlab-Seite zur Verwendung des Plugins.

Der Scan kann auch die {product-name} REST API verwenden, indem das unten bereitgestellte Skript für den Zugriff auf den Controller konfiguriert wird.
...
=== Scannen während der Gitlab-Erstellung mit REST API

Verwenden Sie das folgende Skript, das für Ihre {product-name} -Anmeldedaten konfiguriert ist, um die Schwachstellen-Scans auszulösen.
