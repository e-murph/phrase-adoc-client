...
=== Erstellen von benutzerdefinierten Regeln zur Datei-/Verzeichnisüberwachung
...
Zeigt, dass die Anwendung python den Zugriff auf die Datei im Verzeichnis /var/opt erlernt hat, als der Dienstmodus von node-pod auf Discover eingestellt war. Dies tritt nur auf, wenn die Regel auf Blockieren eingestellt ist und der Dienst sich im Erkennungsmodus befindet.
