...
Suchen Sie in Azure Active Directory > App-Registrierungen > Anwendungsname > Seite Einstellungen die Zeichenfolge Anwendungs-ID. Dies wird verwendet, um die Client-ID in {product-name} einzustellen. Das Client-Geheimnis kann in den Azure-Schlüssel-Einstellungen gefunden werden.
...
Anwendung. Name der Integration: Name für diese Integration. Wählen Sie einen beliebigen Namen Grant Type (ankreuzen):
...
Claims sind Name/Wert-Paare, die Informationen über einen Benutzer sowie Meta-Informationen über den OIDC-Dienst enthalten.
...
Navigieren Sie auf Ihrer Anwendungsintegrationsseite zur Registerkarte "`Assignments`" und vergewissern Sie sich, dass die entsprechenden Zuweisungen aufgeführt sind:
