...
== Verwendung der Befehlszeile {product-name} 

Die Lösung {product-name} unterstützt eine begrenzte Anzahl von Funktionen über das CLI. Die Befehlszeilenschnittstelle wird durch den Manager unterstützt, der wiederum eine RestAPI verwendet, um Befehle an den Controller zu senden. Der Kontrolleur verwaltet dann den/die Durchsetzer in angemessener Weise. Ein vollständiger Satz von Operationen wird durch die REST-API unterstützt, die direkt von der Steuereinheit aus zugänglich gemacht werden kann. Sie können auf die {product-name} CLI zugreifen, indem Sie z. B. den Befehl cli für den Manager oder Allinone eingeben:
...
Dabei ist '`allinone`' der Containername für den Controller. Möglicherweise müssen Sie die Container-ID für den Namen verwenden.

Obwohl die Befehlszeilenschnittstelle (CLI) über den Manager verfügbar ist, empfehlen wir die Verwendung der xref:automation.adoc[REST-API] direkt im Controller für Abfragen und Automatisierung.

=== CLI-Befehlsbeispiele

Hier finden Sie einige der gebräuchlichsten CLI-Befehle:
...
Verwenden Sie denselben Benutzer und dasselbe Passwort, das Sie für die Konsole verwenden.
...
Weitere CLI-Befehle sind unten aufgeführt.

== Befehlszeilenreferenz & Befehle
...
[NOTE]
====
Sniffer-Dateien werden im Verzeichnis /var/neuvector/pcap im Enforcer-Container gespeichert. Vergewissern Sie sich, dass Sie das Volume dem Verzeichnis Ihres Gastrechners oder dem lokalen Systemverzeichnis zuordnen, damit Sie auf die Dateien zugreifen können. Zum Beispiel in der Datei docker-compose '`- /var/neuvector:/var/neuvector`' in Volumes hinzufügen.
====

Um die Paketaufzeichnung auf einem Pod zu starten, müssen Sie die ContainerID kennen, die Sie in das Feld ID_OR_NAME eingeben. Sie können dies mit `show container -c <container_name>` tun. Dann starten Sie den Sniffer mit `request sniffer start <container_id>`. Zum Beispiel,
...
[IMPORTANT]
====
Wenn die Dauer nicht festgelegt ist, müssen Sie die Sniffer-ID ermitteln, um den Sniffer zu stoppen.  Dies geschieht über `show sniffer -c <containerID>`.  Folgen Sie `request sniffer stop <sniffer_ID>`.
====
