...
Es folgt ein Beispiel für die docker-compose-Datei zur Bereitstellung des All-in-one-Containers auf dem ersten Knoten. Da der All-in-One-Container ein Enforcer-Modul enthält, können Anwendungscontainer auf demselben Knoten gesichert werden. Sowohl der Einsatz auf der grünen als auch auf der braunen Wiese wird unterstützt.
