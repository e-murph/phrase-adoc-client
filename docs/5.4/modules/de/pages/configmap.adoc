...
{product-name} unterstützt die automatische Konfiguration mit der Kubernetes ConfigMap-Funktion. Dies ermöglicht die automatisierte Bereitstellung von {product-name} Containern mit den entsprechenden Konfigurationen, Integrationen und anderen Einstellungen.

Die Einstellung 'always_reload: true' kann in jeder ConfigMap yaml hinzugefügt werden, um das Neuladen dieser yaml bei jedem Start des Controllers zu erzwingen (Version 4.3.2+). Andernfalls wird die ConfigMap nur beim ersten Start oder nach einem vollständigen Neustart des Clusters geladen (siehe Abschnitt "Persistente Speicherung" unten).

=== Vollständiges Beispiel {product-name} ConfigMap (initcfg.yaml)

Die aktuelle ConfigMap finden Sie https://raw.githubusercontent.com/neuvector/manifests/main/kubernetes/5.4.0/initcfg.yaml[unter].

Das Muster ist ebenfalls unten abgebildet. Hier finden Sie alle verfügbaren Einstellungen. Bitte entfernen Sie die nicht benötigten Abschnitte und bearbeiten Sie die benötigten Abschnitte. 

[NOTE]
====
Wenn Sie die Configmap in einem Geheimnis verwenden, lesen Sie den Abschnitt unten für Formatierungsänderungen.
====
...
Erstellen Sie dann das ConfigMap-Objekt:
...
=== Beispiele für eine föderierte ConfigMap (fedinitcfg.yaml)

NeuVector v5.4.0 unterstützt Federation-Automatisierung durch die ConfigMap. Nachfolgend finden Sie Beispiele für `fedinitcfg.yaml` Konfigurationen, die je nach Anwendungsfall auf Ihre primären und verwalteten Cluster angewendet werden können.

Beispiel `fedinitcfg.yaml` für den Primärcluster:
...
Beispiel `fedinitcfg.yaml` für verwaltete Cluster:
...
== Sensible Daten mit einem Geheimnis schützen

Wenn sensible Daten in einigen Abschnitten der configmap enthalten sein sollen, kann für die Abschnitte mit sensiblen Daten ein Geheimnis erstellt werden.

Erstellen Sie zum Beispiel die configMap für NICHT-sensible Abschnitte wie passwordProfile und role:
...
Erstellen Sie dann ein Geheimnis für Abschnitte mit sensiblen Daten, wie z. B.:
...
[IMPORTANT]
====
Entfernen Sie das Pipe-Zeichen '|' in jedem Abschnitt, wie unten gezeigt.
====

Beachten Sie die Entfernung des Pipe-Zeichens unten, wenn Sie configmap-Abschnitte in einem Geheimnis verwenden, enabled auf true setzen und den Abschnitt auskommentieren, der in das Geheimnis aufgenommen werden soll.
...
Nach der Bereitstellung des Controllers werden alle Konfigurationsdateien sowohl von configmap als auch von secret im Ordner /etc/config gespeichert.

Beachten Sie, dass das Geheimnis in den standardmäßigen Kubernetes- und xref:kubernetes.adoc#_deploy_suse_security[OpenShift-Controller-Bereitstellungs-yaml-Dateien] unter Volumes genannt wird.

== ConfigMaps und persistente Speicherung

Sowohl die ConfigMaps als auch die Sicherungskopie des xref:production.adoc#_backups_and_persistent_data[persistenten Speichers] werden nur gelesen, wenn ein neuer {product-name} -Cluster bereitgestellt wird oder der Cluster ausfällt und neu gestartet wird. Sie werden bei rollenden Upgrades nicht verwendet.
