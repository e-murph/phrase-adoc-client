...
* Vergewissern Sie sich, dass der Zugriff auf den Manager (IP-Adresse, Port, Route) durch Firewalls erlaubt ist.
...
* Vergewissern Sie sich bei Leistungsproblemen, dass dem Scanner genügend Speicherplatz zum Scannen großer Bilder zugewiesen ist. Außerdem können in der Pod-Richtlinie Mindestanforderungen für CPU und Arbeitsspeicher festgelegt werden, um eine angemessene Leistung bei der Skalierung zu gewährleisten.
* Zulassungskontrolle. Siehe den Abschnitt Fehlerbehebung im Abschnitt Sicherheitsrisiken... -> Zulassungskontrollen.

=== Aktualisierung von

* Verwenden Sie rollende Updates für den Controller. Wenn Sie Hosts neu starten, sollten Sie die Controller überwachen, wenn sie auf andere Hosts verschoben oder auf den neu gestarteten Hosts neu verteilt werden, um sicherzustellen, dass sie starten, dem Controller-Cluster beitreten und sich stabilisieren/synchronisieren können. Wenn alle Hosts auf einmal oder zu schnell neu gestartet werden, kann dies zu unbekannten Zuständen bei den Controllern führen.
...
Diese Protokolle können Probleme mit der Cluster-Konnektivität, Verwaltungsaktionen, Scan-Aktivitäten und andere nützliche Einträge enthalten. Wenn mehrere Steuergeräte in Betrieb sind, kann es notwendig sein, jedes einzelne zu überprüfen. Diese Protokolle können in einer Datei gespeichert und an den {product-name} -Support gesendet werden.

=== Einschalten des Debug-Modus für {product-name} Steuerungen

Bei Problemen, die eine eingehende Untersuchung erfordern, kann der Debug-Modus für die Controller/Allinone aktiviert werden, der detaillierte Informationen protokolliert. Dies kann die Größe der Protokolldateien stark erhöhen, weshalb empfohlen wird, diese Funktion nach dem Sammeln der Daten zu deaktivieren.
