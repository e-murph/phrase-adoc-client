...
** chaîne = "regex"
** string = "!regex"
* value - Une chaîne de caractères sans limites
...
Ensuite, nous pouvons basculer le contexte utilisateur vers user2 avec un privilège de portée plus large, cluster-level-scope.  Cet utilisateur 2 a un Clusterrolebinding qui n'est pas lié à un Namespace, mais qui a une portée au niveau du cluster, et s'associe à la ressource NvClusterSecurityRules.
...
* Permet de modifier le mode de politique d'un groupe cible configuré, par exemple en mode découverte ou surveillance dans un environnement d'essai, ou en mode protection dans un environnement de production.
