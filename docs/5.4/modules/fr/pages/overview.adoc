...
. Sécurité du point final/de l'hôte. Détecte les élévations de privilèges, surveille les processus et l'activité des fichiers sur les hôtes et dans les conteneurs, et contrôle les systèmes de fichiers des conteneurs pour détecter toute activité suspecte.
...
La solution de sécurité des conteneurs d'exécution {product-name} contient quatre types de conteneurs de sécurité : Les *contrôleurs*, les *responsables de l'application des lois*, les *gestionnaires* et les *scanners*. Un conteneur spécial, appelé *Allinone*, est également fourni pour combiner les fonctions de contrôleur, d'exécuteur et de gestionnaire dans un seul conteneur, principalement pour les déploiements natifs de Docker.
