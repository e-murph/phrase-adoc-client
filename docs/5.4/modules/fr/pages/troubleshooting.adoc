...
* Assurez-vous que les modifications apportées aux paramètres yaml requis (par exemple, NodePort ou LoadBalancer) ou aux valeurs Helm sont définies de manière appropriée.
...
* Analyse du registre. La plupart des problèmes sont liés à des erreurs d'authentification du registre ou à l'impossibilité pour le contrôleur d'accéder au registre à partir du cluster.
