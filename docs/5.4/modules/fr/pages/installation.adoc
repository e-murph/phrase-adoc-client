...
[NOTE]
====
Si vous déployez un docker natif (y compris SWARM), assurez-vous qu'il n'y a pas de pare-feu hôte qui bloque l'accès aux ports requis, tel que firewalld. Si cette option est activée, l'interface docker0 doit être ajoutée en tant que zone de confiance pour les hôtes allinone/contrôleur.
====
