...
== Déployer vers un cluster Swarm
...
== Déploiement sur Docker Swarm en mode privilégié
...
Pour certaines configurations de plate-forme, il est possible de déployer les conteneurs {product-name} sans avoir à les exécuter en mode privilégié. La configuration doit permettre d'ajouter des capacités et de définir le profil de l'armure. Notez que Docker DataCenter/UCP et Swarm ne prennent actuellement pas en charge cette fonctionnalité, mais il est toujours possible de déployer {product-name} manuellement à l'aide de Compose ou Run.
