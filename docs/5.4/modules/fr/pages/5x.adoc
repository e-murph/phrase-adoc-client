...
* *NVSHAS-10010*: L'inondation TCP SYN bloque l'entrée, ce qui entraîne un blocage complet du trafic d'entrée.
...
* **NVSHAS-9886**: L'activité du réseau ne résout pas les noms de domaine pour les connexions de groupes externes personnalisés.
* **NVSHAS-9884**: [Node Scan][Container Scan] L'analyse échoue.
...
* **NVSHAS-8682**: Le service CRD webhook doit être déplacé du tableau de bord crd au tableau de bord de l'application.
...
* (Scanner) Détection du paquetage/module `R` dans Ubuntu et Red Hat Enterprise Linux.
...
* Alerte/blocage du trafic dans le même conteneur
 ...
 ** Ajouter des critères pour vérifier les paramètres RBAC à haut risque pour les comptes de service lors du déploiement des pods. Il s'agit notamment des critères "any action of workload resources", "any action on RBAC", "create workload resources", "listing secrets" et "exec into a container".
...
* Violation implicite de l'IP de l'hôte qui n'est pas associée au nœud.
...
* Promotion automatisée des modes de groupe. Promouvoir le mode de protection d'un groupe en fonction du temps écoulé et de critères. Ne s'applique pas aux groupes créés par le CRD. Cette fonction permet à une nouvelle application de fonctionner en mode découverte pendant un certain temps, en apprenant son comportement et en créant, à l'adresse {product-name}, des règles d'autorisation pour le réseau et le processus, puis en passant automatiquement en mode surveillance, puis en mode protection. Critères de découverte et de surveillance : Temps écoulé pour apprendre toute l'activité du réseau et des processus d'au moins un pod en direct dans le groupe. Critère "surveiller pour protéger" : Il n'y a pas d'événement de sécurité (réseau, processus, etc.) dans le délai fixé pour le groupe.
...
Pour les utilisateurs de Helm, mettez à jour vers {product-name} Helm chart 2.0.0 ou plus récent. En cas de mise à jour d'une installation Operator ou Helm sur OpenShift, voir la note ci-dessous.
