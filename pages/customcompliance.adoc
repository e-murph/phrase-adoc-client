...
=== Test Sample script to check if container has root account with no password.
...
==== Test Sample script to check dirty cow file in the container.
...
== Test Creating a custom check script
...
== Test Creating a response rule for compliance report
