= Test_TwoRegistry Scanning Configuration
:revdate: 2025-06-25
...
== Test_TwoConfigure Registry Scanning
...
=== Test_TwoRepository filter examples
...
=== Test_TwoConfiguring Proxy server for Registry

Please go to Settings -> Configuration to configure proxy settings for registry scanning.

=== Test_TwoNative Docker registry (also Quay and Harbor)

==== Test_TwoAdd Native Docker registry
...
==== Test_TwoStart scanning the Docker registry
...
=== Test_TwoRedhat registry

Ref:  https://access.redhat.com/containers

==== Test_TwoAdd Red Hat registry
...
=== Test_TwoOpenshift registry

==== Test_TwoAdd OpenShift registry with username and password
...
==== Test_TwoAdd OpenShift registry with token
...
==== Test_TwoStability issues in Openshift 3.7 Registry
...
==== Test_TwoAdd JFrog Artifactory registry (Docker Access method -- Repository Path)
...
==== Test_TwoAdding JFrog Artifactory registry (Docker Access method -- subdomain)
...
==== Test_TwoAdd JFrog Artifactory registry (Docker Access method -- port)
...
==== Test_TwoAdd SaaS JFrog Artifactory registry (Docker access method -- Port)
...
==== Test_TwoStart Scanning a JFrog Artifactory Registry
...
==== Test_TwoCreate key for container service account

Google Cloud Platform->IAM->Service Account->account with container registry->CreateKey(action)

image:18-gcpiam.png[gcpiam]

==== Test_TwoCopy json file to client machine

==== Test_TwoAdd Google Container Registry from the {product-name} GUI
...
==== Test_TwoStart Scanning a Google Container Registry
...
=== Test_TwoAzure Container Registry

Ref:
https://azure.microsoft.com/en-us/services/container-registry/

==== Test_TwoObtain Azure container username and password as shown below
...
==== Test_TwoAdd Azure Container Registry from the {product-name} GUI
...
==== Test_TwoStart Scanning a Azure Container Registry
...
=== Test_TwoSonatype Nexus Docker registry

Ref:
https://help.sonatype.com/repomanager3/private-registry-for-docker
https://hub.docker.com/r/sonatype/nexus3/

==== Test_TwoAdd Sonatype Nexus Docker registry
...
==== Test_TwoStart scanning Sonatype Nexus Docker registry
...
=== Test_TwoGitlab Container Registry

==== Test_TwoSample GitLab Environmnent Configurations

[,bash]
----
sudo docker run --detach \
  --hostname gitlab \
  --env GITLAB_OMNIBUS_CONFIG="external_url 'http://10.1.7.73:9096'; gitlab_rails['lfs_enabled'] = Test_Twotrue;" \
...
==== Test_TwoObtain Gitlab private token as shown below
...
==== Test_TwoObtaining External and Registry URLs
...
==== Test_TwoAdd Gitlab Registry from the {product-name} Console
...
=== Test_TwoIBM Cloud Container Registry

Ref:  https://www.ibm.com/cloud/container-registry

==== Test_TwoAdd IBM Container registry
...
=== Test_TwoGitHub Container Registry
...
==== Test_TwoAdd the GitHub Container Registry
