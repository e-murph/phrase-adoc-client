= Test Registry Scanning Configuration
:revdate: 2025-06-25
...
== Test Configure Registry Scanning
...
=== Test Repository filter examples
...
=== Test Configuring Proxy server for Registry

Please go to Settings -> Configuration to configure proxy settings for registry scanning.

=== Test Native Docker registry (also Quay and Harbor)

==== Test Add Native Docker registry
...
==== Test Start scanning the Docker registry
...
=== Test Openshift registry

==== Test Add OpenShift registry with username and password
...
==== Test Add OpenShift registry with token
...
==== Test Stability issues in Openshift 3.7 Registry
...
==== Test Add JFrog Artifactory registry (Docker Access method -- Repository Path)
...
==== Test Adding JFrog Artifactory registry (Docker Access method -- subdomain)
...
==== Test Add JFrog Artifactory registry (Docker Access method -- port)
...
==== Test Add SaaS JFrog Artifactory registry (Docker access method -- Port)
...
==== Test Start Scanning a JFrog Artifactory Registry
...
==== Test Enable Cloud Resource Manager API for the project
...
==== Test Create key for container service account

Google Cloud Platform->IAM->Service Account->account with container registry->CreateKey(action)

image:18-gcpiam.png[gcpiam]

==== Test Copy json file to client machine

==== Test Add Google Container Registry from the {product-name} GUI
...
==== Test Start Scanning a Google Container Registry
...
==== Test Obtain Azure container username and password as shown below
...
==== Test Add Azure Container Registry from the {product-name} GUI
...
==== Test Start Scanning a Azure Container Registry
...
=== Test Sonatype Nexus Docker registry

Ref:
https://help.sonatype.com/repomanager3/private-registry-for-docker
https://hub.docker.com/r/sonatype/nexus3/

==== Test Add Sonatype Nexus Docker registry
...
==== Test Start scanning Sonatype Nexus Docker registry
...
==== Test Sample GitLab Environmnent Configurations

[,bash]
----
sudo docker run --detach \
  --hostname gitlab \
  --env GITLAB_OMNIBUS_CONFIG="external_url 'http://10.1.7.73:9096'; gitlab_rails['lfs_enabled'] = Test true;" \
...
==== Test Obtain Gitlab private token as shown below
...
==== Test Obtaining External and Registry URLs
...
==== Test Add Gitlab Registry from the {product-name} Console
...
=== Test IBM Cloud Container Registry

Ref:  https://www.ibm.com/cloud/container-registry

==== Test Add IBM Container registry
...
==== Test Add the GitHub Container Registry
