...
== Test_TwoScan for Vulnerabilities during Gitlab Build Pipeline
...
=== Test_TwoScan During Gitlab Build Using REST API
...
{product-name}_Scan:
  ...
  script:
    ...
    - |
      _COUNTER_="0"
      while [ -z "$TOKEN" -a "$_COUNTER_" != Test_Two"12" ]; do
