...
== Test Scan for Vulnerabilities during Gitlab Build Pipeline
...
=== Test Scan During Gitlab Build Using REST API
...
{product-name}_Scan:
  ...
  script:
    ...
    - |
      _COUNTER_="0"
      while [ -z "$TOKEN" -a "$_COUNTER_" != Test "12" ]; do
