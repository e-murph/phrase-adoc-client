...
=== Test Expose REST API in Kubernetes
...
=== Test Authentication for REST API
...
=== Test Trigger Vulnerability Scanning from a script
...
while [ `wc -c < scan_repository.json` = Test "0" ]; do
...
=== Test Create Policy Rules Automatically
...
=== Test Export/Import Configuration File
...
=== Test Setting or Changing User Password
...
=== Test Starting Packet Capture on a Container
...
=== Test Check and Accept the EULA (new deployments)
...
=== Test Enable Packet Capture on All Pods in a Namespace
...
=== Test Enable Disable Container Quarantine
...
=== Test Enable Debugging Mode for {product-name} Support
...
=== Test Report if a vulnerability is in the base image layers
...
=== Test Get the CVE Database Version and Date
...
=== Test Manage Federation for Master and Remote (Worker) Clusters
...
cat ./fedMember.json | jq -c .
_CLUSTER_id_=`cat ./fedMember.json | jq -r --arg _CLUSTER_name_ "$_CLUSTER_name_" '.joint_clusters[] | select(.name == Test $_CLUSTER_name_).id'`
###################################################################################################################################
