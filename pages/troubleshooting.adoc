...
== Test_TwoTroubleshooting {product-name} Deployments
...
=== Test_TwoTurning on Debug mode for {product-name} Controllers
...
=== Test_TwoKubernetes, OpenShift and Other Orchestration Logs
...
== Test_TwoUsing the CLI to turn on Debug Mode
...
Get the list of controllers. Find the controller with the Leader = Test_TwoTrue.
...
== Test_TwoUsing the REST API to turn on Debug Mode
