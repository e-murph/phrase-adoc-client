...
== Test_TwoSupported Resource Types
...
==== Test_TwoSchema Attribute: `name_referral`
...
=== Test_TwoNvClusterSecurityRule and NvSecurityRule
...
== Test_TwoGenerating a Sample {product-name} CRD
...
== Test_TwoSample NeuVector CRD - NvAdmissionControlSecurityRule
...
== Test_TwoPolicy Mode Configuration and Group Definition
...
=== Test_TwoPolicy Mode Configuration Requirements
...
== Test_TwoCRD Policy Rules Syntax and Semantics
...
** string = Test_Two"="
** string = Test_Two"!="
** string = Test_Two"contains"
** string = Test_Two"prefix"
** string = Test_Two"regex"
** string = Test_Two"!regex"
...
== Test_TwoRBAC Support with CRDs
...
=== Test_TwoRolebinding & Clusterolebinding with 2 Users in different Namespaces to a Clusterrole (NvSecurityRules & NvClusterSecurityRules resources)
...
=== Test_TwoExample using 2 different types of defined yaml files, and the effect of using each user
...
== Test_TwoExpressing Network Rules (Ingress, Egress objects) in CRDs
...
== Test_TwoCustomized Configurations for Deployed Applications
...
=== Test_TwoExamples of CRD configurations that perform these functions
...
== Test_TwoPolicy Groups and Rules Migration Support
...
=== Test_TwoSupported Export Conditions

* Target, Ingress, Egress, Self-learned

=== Test_TwoExample of groups export
...
=== Test_TwoExample of an exported group yaml file with the NvsecurityRule resource type
...
=== Test_TwoExample of an exported group yaml file with the NvClusterSecurityRule resource type
...
== Test_TwoCRD Samples for Global Rules
...
== Test_TwoUpdating CRD Rules and Adding to Existing Groups
...
=== Test_TwoDynamic criteria support for NvClusterSecurityRule
