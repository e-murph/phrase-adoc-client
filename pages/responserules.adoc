...
== Test Detailed Configuration for Response Rules
...
=== Test Response Rule Parameters Matrix

image:resp1.png[matrix]

=== Test Using Multiple Criteria in a Single Rule
...
=== Test Creating a response rule for security event logs
...
=== Test Sample rule to quarantine container and send webhook when package is updated in the nv.alpinepython.default container.

image:resp3.png[rulesample]

=== Test Icons to manage rules - edit, delete, disable and insert new rule below

image:resp4.png[manage]

=== Test Creating a response rule for event logs
...
=== Test Sample events that can be chosen for a response rule

image:resp5.png[events]

=== Test Sample criteria for Admission control events

image:admission.png[events]

=== Test Creating a response rule for cve-report category (log level and report name as matching criteria)
...
=== Test Sample CVE report types that can be chosen for CVE-Report category response rule

image:resp7.png[cvereport]

=== Test Quarantine container and send webhook when vulnerability scan results contain more than 5 high level CVE vulnerabilities for that container

image:resp8.png[cvequarantine]

=== Test Send a webhook if container contains vulnerability with name cve-2018-12

image:resp8-a.png[webhook]

=== Test Creating response rule for CIS benchmarks (log level and benchmark number as matching criteria)
...
=== Test Unquarantine a container by deleting response rule
...
=== Test Viewing the rule id responsible for the container quarantine (in Notifications -> Events)

image:resp9.png[unquarantine]

=== Test Unquarantine option popup when the appropriate response rule is deleted

Check the box to unquarantine any containers which were quarantined by this rule

image:resp10.png[option]

=== Test Complete list of categoried criteria that can be configured for Response Rules
...
==== Test Violations (Security Event)
...
==== Test Dynamically Generated Criteria
