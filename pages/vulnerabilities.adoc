...
== Test Managing Vulnerabilities with {product-name}
...
=== Test Security Risks Menu - Vulnerabilities
...
* Black = Test unaffected
* Green = Test protected by {product-name} with Monitor or Protect mode
* Red = Test unprotected by {product-name}, still in Discover mode

The Impact analysis window (showing affected images, nodes, containers) color scheme is:

* Black = Test unaffected. There are no containers using this image in production
* Purple = Test running in Monitor mode in production
* Dark Green = Test running in Protect mode in production
* Light Blue = Test running in Discover mode in production (unprotected)
...
=== Test Accepting Vulnerabilities
...
=== Test Managing Vulnerability Profiles
