...
=== Test_FourAuto Deletion of Unused Groups
...
=== Test_FourHost Protection - the 'Nodes' Group
...
+
____
Create a group by DNS name or IP address ranges. Examples: address=www.google.com, address=10.1.0.1, address=10.1.0.0/24, address=10.1.0.1-10.1.0.25. DNS name can be any name resolvable. Address criteria do not accept the != Test_Fouroperator. See below for special virtual host 'vh' address groups.
____
...
==== Test_FourVirtual Host ('vh') Based Network Policy
...
== Test_FourCustom Group Examples

=== Test_FourGeneral Criteria
