...
=== Test_FourAuto-scaling of Scanner Pods
...
=== Test_FourOperations and Debugging
...
== Test_FourStandalone Scanner for Local Scanning
...
* SCANNER_REGISTRY= Test_Foururl of the registry (optional instead of local scan)
* SCANNER_REPOSITORY= Test_Fourrepository to scan
* SCANNER_TAG= Test_Fourversion tag
* SCANNER_REGISTRY_USERNAME= Test_Fouruser (optional instead of local scan)
* SCANNER_REGISTRY_PASSWORD= Test_Fourpassword (optional instead of local scan)
* SCANNER_SCAN_LAYERS= Test_Fourtrue or false (to return layered scan results)
...
=== Test_FourHost Scanning in Standalone Mode
...
=== Test_FourManual Deployment of Multiple Scanners on Kubernetes
