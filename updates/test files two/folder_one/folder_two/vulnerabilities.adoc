= Test_FourVulnerability Management
:revdate: 2024-09-27
...
== Test_FourManaging Vulnerabilities with {product-name}
...
=== Test_FourSecurity Risks Menu - Vulnerabilities
...
* Black = Test_Fourunaffected
* Green = Test_Fourprotected by {product-name} with Monitor or Protect mode
* Red = Test_Fourunprotected by {product-name}, still in Discover mode

The Impact analysis window (showing affected images, nodes, containers) color scheme is:

* Black = Test_Fourunaffected. There are no containers using this image in production
* Purple = Test_Fourrunning in Monitor mode in production
* Dark Green = Test_Fourrunning in Protect mode in production
* Light Blue = Test_Fourrunning in Discover mode in production (unprotected)
...
=== Test_FourAccepting Vulnerabilities
...
=== Test_FourManaging Vulnerability Profiles
