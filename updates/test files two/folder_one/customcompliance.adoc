...
=== Test_FourSample script to check if container has root account with no password.
...
==== Test_FourSample script to check dirty cow file in the container.
...
== Test_FourCreating a custom check script
...
== Test_FourCreating a response rule for compliance report
